/*Koriscenjem programskog jezika C napisati Linux program koji u zadatom direktorijumu i njegovim poddirektorijumima (maksimalno do nivoa koji se zadaje 
kao ulazni argument programa, ukoliko se za nivo zada 0 onda se stablo obilazi po citavoj dubini) odredjuje se i stampa prvih 10 linija iz svake regularne datoteke. */

#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
#include <sys/types.h>
#include <dirent.h>
#include <sys/stat.h>

#define MAX_NAME 1024

void processdir(char * path, int level, int maxLevel);

int main(int argc, char * argv[])
{

	char dirname[MAX_NAME];
	char path[MAX_NAME];
	struct stat statbuf;
	DIR * dp;
	struct dirent * dirp;
	int count, size;

	if (argc < 2)
	{
		printf("Nema dovoljno ulaznih argumenata\n");
		exit(-1);
	}

	strcpy(dirname, argv[1]);
	int br = atoi(argv[2]);
	//proveravamo da li je uneta putanja validna
	if (stat(dirname, &statbuf) < 0)
	{
		printf("Doslo je do greske prilikom ocitavanja statusa unete putanje\n");
		exit(-1);
	}

	//da li je uneta putanja do direktorijuma
	if (!S_ISDIR(statbuf.st_mode))
	{
		printf("Uneti argument ne predstavlja putanu do direktorijuma\n");
		exit(-1);
	}

	processdir(dirname, 1,br);

	return 0;
}
void processdir(char * path, int level, int maxLevel)
{
	DIR * dp;
	struct dirent * dirp;
	char path1[MAX_NAME];
	struct stat statbuf;
	dp = opendir(path);
	
	if (dp == NULL)
	{
		printf("Greska prilikom otvaranja direktorijuma\n");
		exit(-1);
	}

	while ((dirp = readdir(dp)) != NULL)
	{
		if (strcmp(dirp->d_name, ".") == 0 || strcmp(dirp->d_name, "..") == 0)
			continue;

		strcpy(path1, path);
		strcat(path1, "/");
		strcat(path1, dirp->d_name);

		stat(path1, &statbuf);

		//ovde sam stavila ./sep21-3.c zbog provere
		if (S_ISREG(statbuf.st_mode) && strstr(path1,".txt")!=NULL)
		{
			if (fork() == 0)
			{
				printf("File: %s\n", path1);
				execlp("head", "head", "-10", path1, NULL);
			}
			else
			{
				wait(NULL);
			}
		}
		else if (S_ISDIR(statbuf.st_mode) && (level < maxLevel || maxLevel == 0))
		{
			processdir(path1, level + 1,maxLevel);
		}
	}
	closedir(dp);
}